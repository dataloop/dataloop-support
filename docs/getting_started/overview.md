...
Tagging can also be done in the UI although this is only recommended for static environments. You want your servers to spin up with the agent installed and with a set of tags that can be used in Outlyer to setup the rest of the monitoring.
...
For large installations we recommend setting up a StatsD server per environment and tagging the agent on the StatsD server with the environment tag it is part of. This then makes metrics browsing easier inside Outlyer.
